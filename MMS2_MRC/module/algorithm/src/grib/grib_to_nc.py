                    ...
                    if len(v) == 1:
                        if k not in list(nc_fid.variables.keys()):
                            if len(v[0]) == 1:
                                ...
                                if 'ens' not in list(nc_fid.dimensions.keys()):
                                    nc_fid.createDimension('ens', None)
                                ...
                                if 'time' not in list(nc_fid.variables.keys()):
                                    vtime = nc_fid.createVariable("time", np.int32, ("time",), fill_value=0)
                                ...
                                if 'ens' not in list(nc_fid.variables.keys()):
                                    vens = nc_fid.createVariable("ens", np.int32, ("ens",))
                                    vens[:] = np.array([0])

                                multivat = nc_fid.createVariable(k, "f8", ("ens", "time", "latitude", "longitude",), fill_value=False)
                                ...
                                for i, vg in enumerate(v[0]):
                                    ...
                                    if k not in list(nc_fid.variables.keys()):
                                        multivat = nc_fid.createVariable(k, "f8", (dimv, "time", "latitude", "longitude",), fill_value=0)
                                    ...
                                    multivat[i, 0, :, :] = vg['values'].data if isinstance(vg['values'], np.ma.core.MaskedArray) else vg['values']
                        ...
                        for i,val in enumerate(v):
                            ...
                            if len(val) == 1:
                                ...
                                if varname not in list(nc_fid.variables.keys()):
                                    multivat = nc_fid.createVariable(varname, "f8", ("ens", "time", "latitude", "longitude",), fill_value=False)
                                ...
                                for j, sval in enumerate(val):
                                    ...
                                    if mulvarname not in list(nc_fid.variables.keys()):
                                        multivat = nc_fid.createVariable(mulvarname, "f8", (mdimv, "time", "latitude", "longitude",), fill_value=0)
                                    ...
                                    multivat[j, 0, :, :] = sval['values'].data if isinstance(sval['values'], np.ma.core.MaskedArray) else sval['values']
...
if __name__ == '__main__':
    ...
    sourcePath = r'./grib.GRB'
    destinationPath = r'./gen/gnc/gncFile12560ii.nc'
    grib_to_nc_batGenNCfile(sourcePath, destinationPath, '')
