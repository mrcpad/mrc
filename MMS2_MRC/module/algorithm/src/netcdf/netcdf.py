...
class Nc(PyMeteoDataInfo):
    ...
    def __get_variable_data_by_name(self, name):
        '''
        get variable data array by name,return OrderedDict，按照起报时间组合
        :param name:
        :return:
        '''
        ...
        if t_variable == None:
            ...
            #2019-11-15更新：不存在预报时间时，键值默认是输入的要素名称
            data_array_dict[name] = variable[::]
            is_has_time = False
        else:
            i = 0
            ...
            for time in num2date(t_variable[::], t_variable.units):
                ...
                i += 1
        #2019-11-15更新：返回值增加是否包含预报时间
        return data_array_dict,is_has_time
    ...
    def get_data_by_name(self, name, **kwargs):
        #2019-11-15更新：返回值增加is_has_time
        data,is_has_time = self.__get_variable_data_by_name(name)
        return data,is_has_time
