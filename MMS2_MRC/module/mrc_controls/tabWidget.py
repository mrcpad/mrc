from PyQt5.QtWidgets import QTabWidget, QWidget, QComboBox,QTableWidgetItem,QPushButton,QAbstractItemView,QVBoxLayout,QFileDialog,QTableWidget,QMessageBox,QApplication
...
class MrcTabWidget(QTabWidget, BaseControl):
    ...
    def tab3UI(self, *args, **kwargs):
        try:
            ...
            if self.flagModel == 'MICAPS4':
                if self.args:
                    self.varName = kwargs.get('fileName', '')
                    self.Micaps()
                ...
                self.varName = args[0]
                messagePos=kwargs.get('messagePos')
                self.GRIB(messagePos)
        except Exception as arg:
            print(arg)
            self.flagExD = 1
            QMessageBox.warning(self, '警告', 'tab3加载异常！', QMessageBox.Yes, QMessageBox.No)
    ...
    def Micaps(self):
        try:
            ...
            print(arg)
            self.flagExD = 1
            QMessageBox.warning(self, '警告', 'Micaps数据显示异常！', QMessageBox.Yes, QMessageBox.No)
        ...
        try:
            ...
            if len(self.args) > 1:
                ...
                if len(self.v.shape) == 1:
                    ...
                    self.latitude = self.args[1].get(self.v.dimensions[0], np.array([str(i + 1) for i in range(self.v.shape[0])]))[:].astype(np.str)
                    self.longitude = self.args[1].get(self.v.dimensions[1], np.array([str(i + 1) for i in range(self.v.shape[1])]))[:].astype(np.str)
                    self.latitude = list(map(lambda x: ('%.2f') % float(x), self.latitude.astype(np.float64)))
                    self.longitude = list(map(lambda x: ('%.2f') % float(x), self.longitude.astype(np.float64)))
                        ...
                        if self.time:
                            if self.time.dtype != np.str:
                                try:
                                    ...
                                    self.timedict = dict(zip([str(tds) for tds in self.time[::].data], range(len(self.time[::]))))
                                    self.comboBox.addItems([str(d) for d in self.timedict.keys()])
                                    if self.comboBox.count() > 0:
                                        self.comboBox.clear()
                                        self.comboBox.addItems([str(d) for d in self.timedict.keys()])
                    ...
                    if 'latitude' in self.v.dimensions or 'lat' in self.v.dimensions or 'lats' in self.v.dimensions:
                        for lat in ['latitude', 'lat', 'lats']:
                            for dim in self.v.dimensions:
                                if lat == dim:
                                    self.latitude = self.args[1].get(lat, self.v.dimensions[1])[:].astype(np.str)
                    else:
                        self.latitude = self.args[1].get(self.v.dimensions[1], np.array([str(i + 1) for i in range(self.v.shape[1])]))[:].astype(np.str)
                    if 'longitude' in self.v.dimensions or 'lon' in self.v.dimensions or 'lons' in self.v.dimensions:
                        for lon in ['longitude', 'lon', 'lons']:
                            for dim in self.v.dimensions:
                                if lon == dim:
                                    self.longitude = self.args[1].get(lon, self.v.dimensions[2])[:].astype(np.str)
                    else:
                        self.longitude = self.args[1].get(self.v.dimensions[2], np.array([str(i + 1) for i in range(self.v.shape[2])]))[:].astype(np.str)
                        ...
                        if self.time:
                            ...
                            if self.time.dtype != np.str:
                                try:
                                    ...
                                    self.timedict = dict(zip([str(tds) for tds in self.time[::]], range(len(self.time[::]))))
                                    self.comboBox.addItems([str(d) for d in self.timedict.keys()])
                                    if self.comboBox.count() > 0:
                                        self.comboBox.clear()
                                        self.comboBox.addItems([str(d) for d in self.timedict.keys()])
                    ...
                    if 'latitude' in self.v.dimensions or 'lat' in self.v.dimensions or 'lats' in self.v.dimensions:
                        for lat in ['latitude', 'lat', 'lats']:
                            for dim in self.v.dimensions:
                                if lat == dim:
                                    self.latitude = self.args[1].get(lat, self.v.dimensions[2])[:].astype(np.str)
                    else:
                        self.latitude = self.args[1].get(self.v.dimensions[2], np.array([str(i + 1) for i in range(self.v.shape[2])]))[:].astype(np.str)

                    if 'longitude' in self.v.dimensions or 'lon' in self.v.dimensions or 'lons' in self.v.dimensions:
                        for lon in ['longitude', 'lon', 'lons']:
                            for dim in self.v.dimensions:
                                if lon == dim:
                                    self.longitude = self.args[1].get(lon, self.v.dimensions[3])[:].astype(np.str)
                    else:
                        self.longitude = self.args[1].get(self.v.dimensions[3], np.array([str(i + 1) for i in range(self.v.shape[3])]))[:].astype(np.str)
            ...
            print(arg)
            self.flagExD = 1
            QMessageBox.warning(self, '警告', 'NetCDF数据显示异常！', QMessageBox.Yes, QMessageBox.No)
    ...
    def HDF(self):
        try:
                        ...
                        if self.time:
                            ...
                            if self.time.dtype != np.str:
                                try:
                                    ...
                                    self.timedict = dict(
                                        zip([str(tds) for tds in self.time[::].data], range(len(self.time[::]))))
                                    self.comboBox.addItems([str(d) for d in self.timedict.keys()])
                                    if self.comboBox.count() > 0:
                                        self.comboBox.clear()
                                        self.comboBox.addItems([str(d) for d in self.timedict.keys()])
            ...
            print(arg)
            self.flagExD = 1
            QMessageBox.warning(self, '警告', 'HDF数据显示异常！', QMessageBox.Yes, QMessageBox.No)


    def GRIB(self,messagePos):
        try:
            self.TableWidget.clear()
            self.TableWidget.setRowCount(0)
            self.TableWidget.setColumnCount(0)
            ...
            dataInfo = self.args[1]

            message = dataInfo.get_message(messagePos)

            #message属性信息
            gribdatatime = message.DateTime
            griblevel = message.Level

            xDimension_net=message.Parameter.XDimension.GetValues()#lon
            yDimension_net=message.Parameter.YDimension.GetValues()#lat

            xDimension=[]
            yDimension=[]
            for i in range(xDimension_net.Length):
                value = str(xDimension_net[i])
                xDimension.append(value)

            for i in range(yDimension_net.Length):
                value = str(yDimension_net[i])
                yDimension.append(value)


            #self.TableWidget.setRowCount(len(yDimension))
            #self.TableWidget.setColumnCount(len(xDimension))

            #self.TableWidget.setHorizontalHeaderLabels(xDimension)


            #self.TableWidget.setShowGrid(True)


            data = dataInfo.get_data_by_name(messagePos)  # 二维数据
            self.TableWidget.setRowCount(data.YNum)
            self.TableWidget.setColumnCount(data.XNum + 1)
            npd = list()

            for i in range(data.YNum):
                for j in range(data.XNum):
                    npd.append(data.Data[i,j])

            npd = np.array(npd).reshape(data.YNum,data.XNum)
            xDimension = [str('%.2f' % float(c)) for c in xDimension]
            yDimension = [str('%.2f' % float(c)) for c in yDimension]
            self.GRIBdf = pd.DataFrame(npd,index=yDimension,columns=xDimension)
            #self.df_original = self.GRIBdf.copy()
            df = self.GRIBdf.copy()
            df['平均值'] = df.mean(axis=1).round(decimals=2)
            df = df.applymap(lambda x: '%.2f' % x)
            columns = [str(c) for c in df.columns.values.tolist()]
            self.TableWidget.setHorizontalHeaderLabels(columns)
            self.TableWidget.setVerticalHeaderLabels(yDimension)
            for i, v0 in enumerate(df.values.tolist()):
                for j, v1 in enumerate(v0):
                    self.TableWidget.setItem(i, j, QTableWidgetItem(str(v1)))
                    QApplication.processEvents()

            self.comboBox.addItem(gribdatatime.ToString("yyyy-MM-dd HH:mm"))
            self.comboBox.setDisabled(True)
            self.comboBox_2.addItem(str(griblevel))
            self.comboBox_2.setDisabled(True)
            self.TableWidget.setEditTriggers(QAbstractItemView.NoEditTriggers)
            self.flagExD = 0
        except Exception as arg:
            print(arg)
            self.flagExD = 1
            QMessageBox.warning(self, '警告', 'GRIB数据显示异常！', QMessageBox.Yes, QMessageBox.No)
    ...
    def chargeTime(self):
        try:
            ...
            print(arg)
            QMessageBox.warning(self, '警告', '改变时间异常！', QMessageBox.Yes, QMessageBox.No)
    ...
    def chargeLevel(self):
        try:
            ...
            print(arg)
            QMessageBox.warning(self, '警告', '改变层次异常！', QMessageBox.Yes, QMessageBox.No)
    ...
    def chargeMean(self):
        try:
            ...
            print(arg)
            self.flagExD = 1
            QMessageBox.warning(self, '警告', '数据分析异常！', QMessageBox.Yes, QMessageBox.No)
    ...
    def exportData(self):
        try:
            if self.flagExD == 0:
                openfileName = QFileDialog.getSaveFileName(self, "选取文件", r"D:\PanoplyWin\{varName}.csv".format(varName=self.varName.replace(':','')),"Binary files(*.nc , *.hdf ,*.GRB, *.grb2);;All Files (*)")
                if openfileName[0]:
                    if self.flagModel == 'MICAPS4':
                        ...
                        df = self.GRIBdf.copy()
                        df.to_csv(openfileName[0])
                    QMessageBox.information(self, '导出提示', '文件导出成功！', QMessageBox.Yes, QMessageBox.No)
            elif self.flagExD == 2:
                QMessageBox.warning(self, '警告', '不支持1D数据导出！', QMessageBox.Yes, QMessageBox.No)
            elif self.flagExD == 1:
                pass
        except Exception as arg:
            print(arg)
            self.flagExD = 1
            QMessageBox.warning(self, '警告', '文件导出异常！', QMessageBox.Yes, QMessageBox.No)
    ...
    def mean(self,fmt=None,model=None):
        try:
            ...
            print(arg)
            self.flagExD = 1
            QMessageBox.warning(self, '警告', '计算平均值异常！', QMessageBox.Yes, QMessageBox.No)
    ...
    def max(self,fmt=None,model=None):
        try:
            ...
            print(arg)
            self.flagExD = 1
            QMessageBox.warning(self, '警告', '计算最大值异常！', QMessageBox.Yes, QMessageBox.No)
    ...
    def min(self,fmt=None,model=None):
        try:
            ...
            print(arg)
            self.flagExD = 1
            QMessageBox.warning(self, '警告', '计算最小值异常！', QMessageBox.Yes, QMessageBox.No)
    ...
    def var(self,fmt=None,model=None):
        try:
            if model == 'MICAPS4':
                df = pd.DataFrame(self.m4data, columns=self.longitude)
                df['方差'] = df.var(axis=1).round(decimals=2)
                if fmt:
                    df = df.applymap(lambda x: fmt % x)
                self.columns = [str(c) for c in df.columns.values.tolist()]
                self.TableWidget.setHorizontalHeaderLabels(self.columns)
            ...
            print(arg)
            self.flagExD = 1
            QMessageBox.warning(self, '警告', '计算方差异常！', QMessageBox.Yes, QMessageBox.No)
    ...
    def std(self,fmt=None,model=None):
        try:
            if model == 'MICAPS4':
                df = pd.DataFrame(self.m4data, columns=self.longitude)
                df['标准差'] = df.std(axis=1).round(decimals=2)
                if fmt:
                    df = df.applymap(lambda x: fmt % x)
                self.columns = [str(c) for c in df.columns.values.tolist()]
                self.TableWidget.setHorizontalHeaderLabels(self.columns)
            ...
            print(arg)
            self.flagExD = 1
            QMessageBox.warning(self, '警告', '计算标准差异常！', QMessageBox.Yes, QMessageBox.No)
    ...
    def median(self,fmt=None,model=None):
        try:
            ...
            print(arg)
            self.flagExD = 1
            QMessageBox.warning(self, '警告', '计算中位数异常！', QMessageBox.Yes, QMessageBox.No)
    ...
    def chargeformat(self):
        try:
            ...
            print(arg)
            self.flagExD = 1
            QMessageBox.warning(self, '警告', '数据格式化异常！', QMessageBox.Yes, QMessageBox.No)
