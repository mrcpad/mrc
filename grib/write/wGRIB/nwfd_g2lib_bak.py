...
class Data2Grib:
    ...
    def data2grib_smiple48(self, discipline, category, element, statistical, status, year, month, day, hour, minute,
                         second, forecasttime, timerange, field, ngrdpts, leveltype, level, isforecast, istimepoint):

        date_time = datetime.datetime(year, month, day, hour, minute, second)  # 起报时间
        ...
        product_type = 2 if isforecast == True else 8  # 2:预报产品    8：实况产品
        # statistical = 0
        # time_range = 3  # 相对于起报时间，预报时间的时间数(该模版中的预报时效为每一段的起始预报时间的时间数，如3小时间隔的预报时间的时间数为0，3，6…)
        # level_type = 103
        # level = 2
